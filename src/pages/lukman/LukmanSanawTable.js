...
import {Button,Space,message,Table,Input,Drawer,Popconfirm,Select} from 'antd';
...
const LukmanTable = props=>{
    ...
    const getBanner = props.getBanner;
            ...
            render: (text, record) => (
                <div size="middle">
                    ...
                    {/* <Popconfirm
                            ...
                            onConfirm={()=>BannereGosh(record)} 
                            // onCancel={cancel}
                            okText="Howa"
                            cancelText="Ýok"
                        > */}
                            <Button onClick={()=>BannereGosh(record)} type='primary' shape='round'   > {dil==="tm"?tm.products.BannereGoş:ru.products.BannereGoş} </Button>                 
                        ...
                        >
                            <Button type='primary' shape='round' danger  > {dil==="tm"?tm.products.Gizle:ru.products.Gizle}  </Button>                 
    ...
    const [showBanner,setShowBaner] = useState(false);
...
const ShowDrawer = async(event)=>{
    edit === true &&  setCategoryId(null);
    edit === true &&  setSubcategoryId(null)
    setEdit(!edit);
...
const EditProduct = async(product_id)=>{
      console.log(sany);
      let sany1 = parseInt(sany);
      console.log(subcategoryId,categoryId,brandId)
  ...
  const BannerdenAyyr = (pro_id,ban_id)=>{
      console.log(pro_id)
        axiosInstance.post("admin/banners/delete-product/"+ban_id,{
               product_id:pro_id
      }).then((data)=>{
          message.success("Üstünlikli aýyryldy!");
          getProducts();
          getBanner();
      }).catch((err)=>{
          console.log(err);
  ...
  const BannerGosh = (pro_id,ban_id)=>{
      axiosInstance.post("admin/banners/add-product/"+ban_id,{
            product_id:pro_id
      }).then((data)=>{
  ...
  const ChangeSubKategory = (value)=>{ 
        setSubcategoryId(value)
  }
  const ChangeKategory = (value)=>{ 
      console.log(value)
    setCategoryId(value);
    let newCategory = categories.filter((category)=>{
    ...
    return(
        <div className='LukmanTable'>
                ...
                onClose={()=>ShowDrawer()}
                visible={edit}>
                    <Select onChange={ChangeKategory} className='suruji-yagdayy--input' value={categoryId}   placeholder={dil==="tm"?tm.products.KategoriýaSaýla:ru.products.KategoriýaSaýla}>
                        {categories?.map((category)=>{  
                        return <Option value={category.id}>{dil==="tm"?category.category_name_tm:category.category_name_ru}</Option>
                        }) }
                    </Select>
                    <Select onChange={ChangeSubKategory} className='suruji-yagdayy--input' value={subcategoryId} placeholder={dil==="tm"?tm.products.SubKategoriýaSaýla:ru.products.SubKategoriýaSaýla}>
                    {subCategories?.map((subCategory)=>{  
                        return <Option value={subCategory.id}>{dil==="tm"?subCategory.subcategory_name_tm:subCategory.subcategory_name_tm}</Option>
                        }) }
                    </Select>
                ...
                width={500}
                className='lukman-table--drawer'
                title={dil==="tm"?tm.products.Bannerler:ru.products.Bannerler}
                placement="right"
                onClose={()=>BannereGosh()}
                visible={showBanner}>
                    <div style={{width:"100%",display:"inline-flex",justifyContent:"space-between"}}>
                        <img style={{width:"200px",height:"200px",margin:"10px auto",objectFit:"cover"}} src={maglumat && maglumat.product_image && (BASE_URL+"/"+maglumat.product_image)} alt="product Surat"/>
                        <h3 style={{width:"200px",textAlign:"center"}}>{maglumat && maglumat.product_name_tm && maglumat.product_name_tm}</h3>
                    </div>
                    <hr />
                    <div style={{width:"500px"}}>

                            {
                                banner && banner.map((ban)=>{
                                    return <div style={{width:"480px",display:"inline-flex",justifyContent:"space-around"}}>
                                       <img style={{width:"200px",height:"200px",objectFit:"contain",display:"block"}} src={BASE_URL+"/"+ban.banner_image} />

                                       {maglumat && maglumat.bannerId ==ban.id?
                                       <Button danger style={{width:"150px",marginRight:"5px",marginTop:"50px"}} onClick={()=>BannerdenAyyr(maglumat.product_id,ban.banner_id)} icon={<PlusCircleFilled/>} shape='round' type='primary' className='suruji-yagdayy--button'>{dil==="tm"?tm.products.Bannerden_aýyr:ru.products.Bannerden_aýyr}</Button>
                                        : <Button style={{width:"150px",marginRight:"5px",marginTop:"50px"}} onClick={()=>BannerGosh(maglumat.product_id,ban.banner_id)} icon={<PlusCircleFilled/>} shape='round' type='primary' className='suruji-yagdayy--button'>{dil==="tm"?tm.products.Bannere_goş:ru.products.Bannere_goş}</Button>}
                                        </div>
                                })
                            }
